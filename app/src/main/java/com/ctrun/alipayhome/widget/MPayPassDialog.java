...
public class MPayPassDialog {
     ...
     * @param context
     */
    public MPayPassDialog(Context context) {
     ...
     */
    public MPayPassDialog(Context context, int themeResId) {
        this.mContext = context;
     ...
     * 初始化Dialog
     */
    public MPayPassDialog setAlertDialog(){
        mDialog=new AlertDialog.Builder(mContext,mThemeResId).create();
        mDialog.setCancelable(true);//按返回键退出
        mDialog.show();
        return this;
    }
    public MPayPassDialog setAlertDialog(boolean isBack){
        mDialog=new AlertDialog.Builder(mContext,mThemeResId).create();
     ...
     * 设置弹框大小 透明度
     */
    public MPayPassDialog setWindowSize(int width, int height,float amount){
     ...
     * custom=其他   指定高度
     */
    public MPayPassDialog setWindowSize(int width, int height,int custom,float amount){
        if(custom==2){
            mDialog.getWindow().setDimAmount(amount);//设置透明度
            window = mDialog.getWindow();
     ...
     * 点击外部消失
     */
    public MPayPassDialog setOutColse(boolean isOut){
        if (isOut){
     ...
     * 方式 与位置
     */
    public MPayPassDialog setGravity(int animation, int gravity){
